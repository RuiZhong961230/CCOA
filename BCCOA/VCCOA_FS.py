...
def Tanh(indi):
    ...
    for i in range(len(indi)):
        if np.random.rand() >= abs(np.tanh(indi[i])):
            new_indi[i] = 1
    return new_indi
...
def RunCCOA(func):
    ...
    np.savetxt("./VCCOA_Data/FS/Acc/" + Func_num + ".csv", All_Trial_Best, delimiter=",")
    np.savetxt("./VCCOA_Data/FS/Scale/" + Func_num + ".csv", All_Scale, delimiter=",")
...
def main():
    ...
    Datasets = ['Horse', 'Ionosphere', 'Sonar', 'Soybean-small', 'SpectEW',
                'wdbc', 'Wine', ]
    Func_num = 0
...
if __name__ == "__main__":
    if os.path.exists('./VCCOA_Data/FS/Acc') == False:
        os.makedirs('./VCCOA_Data/FS/Acc')
    if os.path.exists('./VCCOA_Data/FS/Scale') == False:
        os.makedirs('./VCCOA_Data/FS/Scale')
    main()
